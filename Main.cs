...
namespace TranslatedTextures
{
    public class Main : MelonMod
    {
            ...
            foreach (MeshRenderer renderer in root.GetComponentsInChildren<MeshRenderer>(true))
            {
                ...
                foreach (var rule in Rules)
                {
                    ...
                    for (int i = 0; i < mats.Length; i++)
                    {
                        ...
                        MelonLogger.Msg($"[ApplyTexturesToObject] '{objName}' -> '{rule.TextureFile}'");

                        if (rule.DamageTextureFile != null &&
                            LoadedTextures.TryGetValue(rule.DamageTextureFile, out Texture2D dmgTex))
        ...
        private static List<TextureRule> BuildRules()
        {
            ...
            foreach (var def in CannedDefs)
            {
                ...
                {
                    rules.Add(new TextureRule { TextureFile = def.TextureFile, TargetName = mesh, ExactName = true, ParentName = def.ParentName, SourceTextureName = def.SourceTextureName });
                    if (def.LidTextureFile != null && def.LidSourceName != null)
                        rules.Add(new TextureRule { TextureFile = def.LidTextureFile, TargetName = mesh, ExactName = true, ParentName = def.ParentName, SourceTextureName = def.LidSourceName });
                }
            }
            rules.AddRange(new[]
            {
                ...
                new TextureRule { TextureFile = "GEAR_FoodCandyBars_Dif.png",TargetName = "CandyBarMesh" },
                ...
                new TextureRule { TextureFile = "OBJ_PotassiumNitrate_Dif.png", TargetName = "ScrapMetal_LOD0", ExactName = true, ParentName = "GEAR_StumpRemover" },
                ...
                new TextureRule { TextureFile = "OBJ_BookHardcover_Cooking.png",           TargetName = "Mesh", ExactName = true, ParentName = "GEAR_BookCooking",             SourceTextureName = "OBJ_Book_Cooking_Mat" },
                new TextureRule { TextureFile = "OBJ_BookSoftcover_Frontier.png",          TargetName = "Mesh", ExactName = true, ParentName = "GEAR_BookRifleFirearm",         SourceTextureName = "OBJ_Book_Frontier_Mat" },
                new TextureRule { TextureFile = "OBJ_BookSoftcover_IceFishing.png",        TargetName = "Mesh", ExactName = true, ParentName = "GEAR_BookIceFishing",           SourceTextureName = "OBJ_Book_Fishing_Mat" },
                new TextureRule { TextureFile = "OBJ_BookMagazine_Guns.png",               TargetName = "Mesh", ExactName = true, ParentName = "GEAR_BookRifleFirearmAdvanced", SourceTextureName = "OBJ_Book_Guns_Mat" },
                new TextureRule { TextureFile = "OBJ_BookGunsmithing_A.png",               TargetName = "Mesh", ExactName = true, ParentName = "GEAR_BookGunsmithing",          SourceTextureName = "OBJ_BookGunsmithing_A" },
                new TextureRule { TextureFile = "OBJ_BookHardcover_Mending.png",           TargetName = "Mesh", ExactName = true, ParentName = "GEAR_BookMending",              SourceTextureName = "OBJ_Book_Mending_Mat" },
                new TextureRule { TextureFile = "OBJ_BookMagazine_Archery.png",            TargetName = "Mesh", ExactName = true, ParentName = "GEAR_BookArchery",              SourceTextureName = "OBJ_Book_Archery_Mat" },
                new TextureRule { TextureFile = "OBJ_BookHardcover_FieldDressingVol1.png", TargetName = "Mesh", ExactName = true, ParentName = "GEAR_BookCarcassHarvesting",    SourceTextureName = "OBJ_Book_Harvesting_Mat" },
                new TextureRule { TextureFile = "OBJ_BookHardcover_Survive.png",           TargetName = "Mesh", ExactName = true, ParentName = "GEAR_BookFireStarting",         SourceTextureName = "OBJ_Book_FireStarting_Mat" },
                new TextureRule { TextureFile = "OBJ_Sign_C.png", TargetName = "OBJ_RoadSignI_LOD0", ExactName = true, SourceTextureName = "OBJ_Sign_C01" },
                new TextureRule { TextureFile = "OBJ_Sign_C.png", TargetName = "OBJ_RoadSignI_LOD1", ExactName = true, SourceTextureName = "OBJ_Sign_C01" },
        ...
        private void LoadAllTextures()
        {
            ...
            foreach (var rule in Rules)
            {
                if (!LoadedTextures.ContainsKey(rule.TextureFile))
                {
                ...
                if (rule.DamageTextureFile != null && !LoadedTextures.ContainsKey(rule.DamageTextureFile))
                {
                    string dmgPath = $"TranslatedTextures.Resources.Textures.Russian.{rule.DamageTextureFile}";
                    Texture2D dmgTex = LoadEmbeddedTexture(dmgPath, Path.GetFileNameWithoutExtension(rule.DamageTextureFile));
                    if (dmgTex != null) LoadedTextures[rule.DamageTextureFile] = dmgTex;
                }
            }
        ...
        private Texture2D LoadEmbeddedTexture(string resourceName, string textureName)
        {
            ...
            {
                if (stream == null) { LoggerInstance.Error($"[TranslatedTextures] ❌ Not found: {resourceName}"); return null; }
        ...
        {
            if (sceneName.Contains("_") || sceneName == "MainMenu" || sceneName == "MainMenu_DLC01" || sceneName == "Boot" || sceneName == "Empty") return;
            StaticReplaceTextures(sceneName);
        }
        ...
        internal static void StaticReplaceTextures(string sceneName)
        {
            ...
            foreach (MeshRenderer renderer in UnityEngine.Object.FindObjectsOfType<MeshRenderer>(true))
            {
                ...
                foreach (var rule in Rules)
                {
                    bool nameMatch = rule.ExactName ? objName == rule.TargetName : objName.Contains(rule.TargetName);
                    if (!nameMatch) continue;
                    if (rule.ParentName != null)
                    {
                        Transform parent = renderer.transform.parent;
                        bool parentFound = false;
                        while (parent != null) { if (parent.name == rule.ParentName) { parentFound = true; break; } parent = parent.parent; }
                        if (!parentFound) continue;
                    }
                    if (!LoadedTextures.TryGetValue(rule.TextureFile, out Texture2D newTex)) continue;
                    Material[] mats = renderer.sharedMaterials;
                    ...
                    for (int i = 0; i < mats.Length; i++)
                    {
                        ...
                        if (rule.SourceTextureName != null)
                        {
                            ...
                            Texture currentTex = mats[i].mainTexture;
                            string texName = currentTex != null ? currentTex.name : "";
                            if (!matName.Contains(rule.SourceTextureName) && !texName.Contains(rule.SourceTextureName)) continue;
                        }
                        mats[i].mainTexture = newTex;
                        changed = true;
                        if (rule.DamageTextureFile != null && LoadedTextures.TryGetValue(rule.DamageTextureFile, out Texture2D dmgTex))
                            mats[i].SetTexture(711, dmgTex);
                    }
    ...
    internal static class GearItem_Awake_Patch
    {
        static void Postfix(Il2Cpp.GearItem __instance)
        {
            ...
            MelonLogger.Msg($"[DIAG] GearItem.Awake: '{name}' scene='{UnityEngine.SceneManagement.SceneManager.GetActiveScene().name}'");
        }
        ...
        internal static class GearItem_OnEnable_Patch
        {
            static void Postfix(Il2Cpp.GearItem __instance)
            {
                if (__instance == null) return;
                string name = __instance.gameObject.name;
                MelonLogger.Msg($"[DIAG] GearItem.OnEnable: '{name}'");
                Main.ApplyTexturesToObject(__instance.gameObject);
            }
        }

        static bool IsInteresting(string n) => n.Contains("Canned") || n.Contains("GEAR_") || n.Contains("Food") || n.Contains("Ammo");
    }
    ...
    internal static class UITexture_SetMainTexture_Patch
    {
        static void Prefix(Il2Cpp.UITexture __instance, ref Texture value)
        {
            ...
            value = replacement;
            if (__instance.gameObject.name == "TextureWithDamage")
            {
                var mat = __instance.mMat;
                if (mat != null && Main.LoadedTextures.TryGetValue("CLTH_TOR_CVR_TShirtSnappy_dmg.png", out Texture2D dmgTex))
                    mat.SetTexture(784, dmgTex);
            }
        }
    }
    ...
    internal static class Panel_Container_Enable_Patch
    {
        static void Postfix()
        {
            MelonLogger.Msg("[DIAG] Panel_Container.Enable fired");
            string sceneName = UnityEngine.SceneManagement.SceneManager.GetActiveScene().name;
            Main.ScheduleReplaceAfterDelay(sceneName);
        }
    }
